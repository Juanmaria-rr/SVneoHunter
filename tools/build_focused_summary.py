...
`docs/EXECUTIVE_SUMMARY.md`, which stays canonical.
...
def strip_branch_suffix(text: str, branch: str) -> str:
    ...
    In a one-branch document `RPE1-WT_noPON` is just RPE1-WT, and carrying the
    suffix invites the opposite of the intended reading: a suffixed name looks
    like a *variant* of the sample, as if there were another RPE1-WT elsewhere
    ...
    patterns = [
        (rf"(?<=[A-Za-z0-9])_{escaped}\b", ""),          # RPE1-WT_noPON
        (rf"<br>\s*`?{escaped}`?", ""),                  # WT<br>noPON
        (rf"\s*\(\s*`?{escaped}`?\s*\)", ""),            # ... (`noPON`)
        (rf"(?<=[A-Za-z0-9])\s+{escaped}\b(?=\s*\|)", ""),  # `RPE1-WT noPON |`
...
def main() -> None:
    ...
    parser.add_argument("--source", default="docs/EXECUTIVE_SUMMARY.md")
    parser.add_argument("--out", default="docs/EXECUTIVE_SUMMARY_focused.md")
    ...
    canonical = "docs/EXECUTIVE_SUMMARY.md"
    note = (
