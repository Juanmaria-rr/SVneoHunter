...
are stated as rules rather than implied by the code.
    ...
    Three events called STRONG by one workflow, over the same BAM:
        NTRK1   BND       cov bp1=3, bp2=8,913, crossing reads=0   -> NONE
        GOLGA3  DEL 2 bp  cov 468/467,          crossing reads=0   -> test invalid
        ITGA11  DEL 220bp cov 2,372/2,367,      crossing reads=31  -> STRONG
    Three STRONG, zero survivors: two had nothing crossing, and the third is a
    common germline polymorphism (PON_COUNT 3,513), i.e. real but not private.
...
RULE 2  NEVER max() COVERAGE ACROSS BREAKENDS.
        Reporting max(bp1, bp2) turned NTRK1's silent chr1 locus into "4,800
        reads" — the count came from its chr8 partner. Both ends are reported and
