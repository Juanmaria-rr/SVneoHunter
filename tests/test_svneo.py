...
def test_admitted_vcf_selects_by_id_not_coordinate():
    ...
    passed — one with PON_COUNT 1,578 reached the peptide generator that way."""
    import io, tempfile, pandas as pd
    from svneo import vcf as vcf_mod
    body = ("##fileformat=VCFv4.2\n"
            "chr17\t75803127\t26224\tN\t]chr17:75803164]CAG\t50\tPASS\tSVTYPE=BND\n"
            "chr17\t75803127\t26225\tN\t]chr17:75803021]CAG\t50\tPASS\tSVTYPE=BND\n")
    with tempfile.NamedTemporaryFile("w", suffix=".vcf", delete=False) as fh:
        fh.write(body)
        source = fh.name
    admitted = pd.DataFrame([{"chrom": "17", "pos": 75803127, "id": "26225"}])
    with tempfile.NamedTemporaryFile("w", suffix=".vcf", delete=False) as fh:
        out = fh.name
